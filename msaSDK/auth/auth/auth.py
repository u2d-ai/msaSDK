...
class AuthBackend(AuthenticationBackend, Generic[_UserModelT]):
    ...
    ) -> Tuple["Auth", Optional[_UserModelT]]:
...
class Auth(Generic[_UserModelT]):
    ...
    async def authenticate_user(
        ...
        if user:
            ...
            if self.pwd_context.verify(
                pwd, pwd2
            ):  # User exists and password validation passes
                return user
        return None
    ...
    def get_current_user(self):
        async def _get_current_user(
            request: Request,
            session: Union[Session, AsyncSession, None] = Depends(
                self.db.session_generator
            ),
        ) -> Optional[_UserModelT]:
            ...
            if token_data is not None:
                request.scope["user"]: _UserModelT = await self.db.async_get(
                    self.user_model, token_data.id, session=session
                )
            return request.user
...
class AuthRouter(MSARouterMixin):
    auth: Auth = None
    ...
    def __init__(self, auth: Auth = None):
        ...
        self.router.add_api_route(
            ...
            dependencies=None,
            response_model=MSACRUDOut[self.schema_user_info],
        )
        self.router.add_api_route(
            "/logout",
            self.route_logout,
            ...
            dependencies=None,
            response_model=MSACRUDOut,
        )
        # oauth2
        self.router.dependencies.append(
            Depends(
                self.OAuth2(tokenUrl=f"{self.router_path}/gettoken", auto_error=False)
            )
        )
        self.router.add_api_route(
            "/gettoken",
            ...
            response_model=MSACRUDOut[self.schema_user_login_out],
        )
    ...
    def route_userinfo(self):
        ...
        async def userinfo(request: Request):
            return MSACRUDOut(data=request.user)
    ...
    def route_gettoken(self):
        async def oauth_token(
            ...
            if request.scope.get("user") is None:
                request.scope["user"] = await request.auth.authenticate_user(
            ...
            token_info.access_token = (
                await request.auth.backend.token_store.write_token(request.user.dict())
            )
            response.set_cookie("Authorization", f"bearer {token_info.access_token}")
            return MSACRUDOut(data=token_info)
