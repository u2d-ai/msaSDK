...
from msaSDK.admin.frontend.components import (Action, ActionType,
...
class UserLoginFormAdmin(FormAdmin, ABC):
    ...
    async def handle(
        self, request: Request, data: MSABaseModel, **kwargs  # self.schema
    ) -> MSACRUDOut[MSABaseModel]:  # self.schema_submit_out
        if request.user:
            return MSACRUDOut(
                code=1,
                msg=_("User logged in!"),
                data=self.schema_submit_out.parse_obj(request.user),
            )
    ...
    def route_submit(self):
        async def route(
            response: Response, result: MSACRUDOut = Depends(super().route_submit)
        ):
    ...
    async def handle(
        ...
        try:
            user.id = await auth.db.async_execute(
                stmt, on_close_pre=lambda r: getattr(r, "lastrowid", None)
            )
...
class UserInfoFormAdmin(FormAdmin):
    ...
    async def handle(
        ...
        stmt = (
            ...
            .where(self.user_model.username == request.user.username)
            .values(data.dict())
        )
...
class UserAdmin(ModelAdmin):
    ...
    async def on_update_pre(
        ...
        if password:
            data["password"] = request.auth.pwd_context.hash(
                data["password"]
            )  # 密码hash保存
        return data
